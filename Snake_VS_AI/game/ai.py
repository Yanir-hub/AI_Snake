...
class SnakeAI:
    ...
    def evaluate_move(self, pos, apple_pos):
        ...
        # Consider power-ups based on difficulty
        for item in self.get_visible_items():
            if item['type'] == 'power_up':
                item_distance = self.heuristic(pos, item['pos'])
                if self.difficulty == 'easy':
                    # Easy AI rarely goes for power-ups
                    if random.random() < 0.2:
                        score -= item_distance
                elif self.difficulty == 'medium':
                    # Medium AI goes for power-ups if they're convenient
                    if item_distance < apple_distance:
                        score -= item_distance * 1.5
                else:  # hard
                    # Hard AI strategically pursues power-ups
                    # Prioritize speed-ups when losing
                    if (self.snake.score < self.target.score and 
                        item['power_type'] == 1):  # Speed Up
                        score -= item_distance * 3
                    # Prioritize freeze/slow when opponent is ahead
                    elif (self.target.score > self.snake.score and 
                          item['power_type'] in [0, 2]):  # Freeze or Slow
                        score -= item_distance * 2
    ...
    def get_visible_items(self):
        """Return list of visible power-ups and apples in the game"""
        items = []
        # Add power-ups if they exist
        if hasattr(self.snake, 'power_ups'):
            for power_up in self.snake.power_ups:
                items.append({
                    'type': 'power_up',
                    'pos': power_up.position,
                    'power_type': power_up.type_id
                })
        return items